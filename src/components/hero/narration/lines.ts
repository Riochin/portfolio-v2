 ...
 *
 * 先頭の 1 本 (WISH_OPENERS) は願い事ではなく、出だし専用の驚き。
 * 残りが願い事の袋で、大きい願いのあとに小さい願いが来る落差が笑いどころ
 * なので、生活まわりの小さいものを必ず混ぜること。
 */
export const WISHES = [
...
/**
 * 先頭の何本が、願い事ではない出だし専用か。
 *
 * この本数ぶんは抽選の袋に入れず、全画面に入って最初の 1 本にだけ決め打つ
 * ── 抽選任せだと、いきなり願い事から始まる回や、落ち (WISH_MISSES) から
 * 始まる回ができて、この人が流れ星に何を言う人なのか分からないまま終わる。
 * 袋から外れているので、続きの一言 (WISH_FOLLOWUP) が付くこともないし、
 * 2 本目でまた同じ驚き方をすることもない。
 */
export const WISH_OPENERS = 1;
