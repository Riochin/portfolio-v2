...
export const BIRDS = {
  ...
  /** 上下のゆれ幅 */
  bob: 1.4,
  /** 群れを組む方位(rad)。54.4°。wide は ±42.8°、narrow は yaw -0.12 のぶん
      ずれて -27.9°〜+41.6° なので、ここまで取ればどちらの画枠でも枠の外で
      組んで外へ抜けられる。折り返しの位置は奥行きに応じて
      |z| * tan(この角) で取る (SHIP.span と同じ持ち方)。
      画角より広ければよく、助走の長さには効かない (Birds.tsx が枠の縁を
      カメラで探して、そこから lead 秒手前に置き直すため) */
  span: 0.95,
  /** 枠に入る手前で飛ばせておく秒数。gap / firstGap からはこのぶんを引く
      ので、待ちの合計は変わらない。ここを詰めると、目いっぱい左を覗いて
      いる人には縁の外の助走が見えてしまう */
  lead: 2,
  /** 出入りの濃さの立ち上がり / 落ち(秒)。助走 (lead) の中で出切るので、
      枠に入るころには濃さは 1 になっている */
  fade: 0.6,
  /** 羽ばたきの速さ(rad/s) */
